...
# 모델 M1을 학습하라 했으므로
M1 = LinearRegression()
M1.fit(data[['volatile acidity', 'chlorides', 'free sulfur dioxide', 'total sulfur dioxide', 'pH', 'sulphates', 'alcohol']], data['quality'])
...
y_pred_M1 = M1.predict(data[['volatile acidity', 'chlorides', 'free sulfur dioxide', 'total sulfur dioxide','pH', 'sulphates', 'alcohol']])
y_true_M1 = data['quality'].values

SSE = np.sum((y_true_M1 - y_pred_M1)**2)
n = len(data)
p = data.shape[1] - 1 - 2 -2 #변수 네 개를 뺐음.
MSE = SSE/(n-p-1)


X= data[['volatile acidity','chlorides', 'free sulfur dioxide', 'total sulfur dioxide','pH', 'sulphates', 'alcohol']].values
X.shape[1]
X = np.c_[np.ones(n), X]
XtX = np.matmul(X.T, X)
...
x_data = data[['volatile acidity','chlorides', 'free sulfur dioxide', 'total sulfur dioxide', 'pH', 'sulphates', 'alcohol']] #변수 여러개
target = data[["quality"]]
